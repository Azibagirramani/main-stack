...
describe("search function", () => {
  ...
  it("filters payments based on date range and status", () => {
    const filter: IFilters = {
      date_range: {
        from: new Date("2022-02-01"),
        to: new Date("2022-03-03"),
      },
      status: ["successful"],
      type: [],
      day: "",
    };
    const result = search(mockPayments, filter);
    expect(result).toHaveLength(7);
  });

  it("filters payments based on status", () => {
    const filter: IFilters = {
      date_range: {
        from: new Date("2022-02-01"),
        to: new Date("2022-03-03"),
  ...
  it("filters payments based on type", () => {
    const filter: IFilters = {
      date_range: {
        from: new Date("2022-02-01"),
        to: new Date("2022-03-02"),
      },
      status: [],
      type: ["withdrawal"],
      day: "",
  ...
  it("filters payments based on day", () => {
    const filter: IFilters = {
      date_range: {
        from: new Date("2022-02-01"),
        to: new Date("2022-03-02"),
  ...
  it("filters payments with no filters", () => {
    const filter: IFilters = {
      date_range: {
        from: new Date("2022-02-01"),
        to: new Date("2022-03-03"),
